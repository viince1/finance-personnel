<template>
  <div class="modal-card" id="ajout-compte">
      <div class="modal-card-head">
        <p class="modal-card-title">Cr&eacute;ation d'un budget</p>
        <button
          type="button"
          class="delete"
          @click="$emit('close')"/>
    ...
    <div class="modal-card-foot">
        <div class="field is-grouped">
          <div class="control">
            <button class="button is-link" @click="addBudget">Ajouter</button>
          </div>
        </div>
      </div>
  </div>
...
export default {
  ...
  methods: {
    async addBudget() {
      ...
      await this.$store.dispatch('budget/createBudget', { budget: this.budget });
      this.$emit('close');
    },
  },
};
